...
void ABoard::SetearPosicionInicial()
{
    ...
    case 1:
        if (CurrentPiece) {
            ...
            FVector MoverLocalizacionY(0.0, 20, 0);
            FVector MoverLocalizacionZ(0.0, 0, -0.5);
            auto locActor = CurrentPiece->GetActorLocation();
            float auxY = locActor.Y;
            float auxZ = locActor.Z;
            if (auxY < 20.0f && flagPermitirMovimiento==false) {
    ...
    case 2:
        if (CurrentPiece) {
            if (flagInicioFuncion == true) {
                Location = FVector(0.0, 200, 285);
                CurrentPiece->SetActorLocation(Location);
                flagInicioFuncion = false;
            }
            FVector MoverLocalizacionY(0.0, -30, 0);
            FVector MoverLocalizacionZ(0.0, 0.0 ,-0.2);
            auto locActor = CurrentPiece->GetActorLocation();
            float auxY = locActor.Y;
            float auxZ = locActor.Z;
            if (auxY > 20.0f && flagPermitirMovimiento == false) {
